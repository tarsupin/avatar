...
if(Form::submitted("upload-item"))
{
	...
	if($results)
	{
		...
		// delete colors from cache
		Cache::delete("color:" . substr(md5($itemData['position'] . $itemData['title']), 0, 20));
	}
}
...
if(Form::submitted("ep-item"))
{
	...
	if($_POST['ep'] > 0)
	{
		...
		{
			Database::query("INSERT INTO packages_content (item_id, package_id) VALUES (?, ?)", array($url[2], (int) $exists['id']));
		}
	}
}
...
echo '</select> layer</p>
		...
		<p><select name="rarity"><option value="0"' . ($itemData['rarity_level'] == 0 ? ' selected' : '') . '>freely purchasable</option><option value="1"' . ($itemData['rarity_level'] == 1 ? ' selected' : '') . '>not purchasable</option><option value="2"' . ($itemData['rarity_level'] == 2 ? ' selected' : '') . '>EP/UJ item</option></select> rarity</p>
