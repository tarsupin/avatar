...
if(isset($url[1]))
{
	...
	foreach ($wished as $itemData)
	{
		...
		<tr' . ($own ? ' class="opaque"' : "") . '>
			...
			<td>' . ($itemData['cost'] !== false ? '<a href="/view-wishlist?buy=' . $itemData['item_id'] . '&' . Link::prepare("purchase-wish") . '" onclick="return confirm(\'Are you sure you want to buy ' . $itemData['title'] . '?\');">' . $itemData['cost'] . ' Auro</a>' : 'Preview Only') . '</td>
